...
public class TextMatrixCommand {
    ...
    private static int execute(CommandContext<CommandSourceStack> context, ParticleOptions effect, Vec3 pos, Component text, double scaling, String matrixStr, double dpb, double size, Vec3 speed, int lifetime, String speedExpression, double speedStep, String group) {
        PacketDistributor.sendToPlayersInDimension(context.getSource().getLevel(), new TextMatrixPayload(effect, pos, text, scaling, matrixStr, dpb, size, speed, lifetime, speedExpression, speedStep, group));
        return 1;
    }
}