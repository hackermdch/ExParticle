...
public class TextMatrixCommand {
    ...
    private static int execute(CommandContext<CommandSourceStack> context, ParticleOptions effect, Vec3 pos, Component text, double scaling, double size, String matrixStr, double dpb, Vec3 speed, int lifetime, String speedExpression, double speedStep, String group) {
        PacketDistributor.sendToPlayersInDimension(context.getSource().getLevel(), new TextMatrixPayload(effect, pos, text, scaling, size, matrixStr, dpb, speed, lifetime, speedExpression, speedStep, group));
        return 1;
    }
}